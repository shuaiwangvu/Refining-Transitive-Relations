...
def compute_strongly_connected_component():
    ...
    file =  open('broader_node_color.csv', 'w', newline='')
    ...
    # export the edges
    file =  open('broader_edges.csv', 'w', newline='')
    writer = csv.writer(file)
    ...
    # export the edges
    file =  open('broader_edges_shortname.csv', 'w', newline='')
    writer = csv.writer(file)
    writer.writerow([ "Subject", "Object"])
    for (l,r) in graph.edges:
...
def draw_graph():
    ...
    # nx.draw(graph)
    plt.savefig("broader_all_graph.png")
    plt.show()
