...
PATH_LOD = './isPartOf.hdt'
broader = "http://purl.org/dc/terms/isPartOf"
...
def init_nodes():
    ...
    global can_remove
    (subclass_triples, cardinality) = hdt_file.search_triples('', broader, '')
...
def construct_graph():
    ...
    for n in collect_nodes:
        (subclass_triples, cardinality) = hdt_file.search_triples('', broader, n)
        for (s, _, _) in subclass_triples:
            if s in collect_nodes:
                if s != n :
                    graph.add_edge(s, n)
        (subclass_triples, cardinality) = hdt_file.search_triples(n, broader, '')
...
def compute_strongly_connected_component():
    ...
    file =  open('isPartOf_node_color.csv', 'w', newline='')
    ...
    # export the edges
    file =  open('isPartOf_edges.csv', 'w', newline='')
    writer = csv.writer(file)
    ...
    # export the edges
    file =  open('isPartOf_edges_shortname.csv', 'w', newline='')
    writer = csv.writer(file)
    writer.writerow([ "Subject", "Object"])
    for (l,r) in graph.edges:
...
def draw_graph():
    ...
    # nx.draw(graph)
    plt.savefig("isPartOf_all_graph.png")
    plt.show()
