...
PATH_LOD = './broaderTransitive.hdt'
...
def is_leaf_node (n):
    (_, s_cardinality) = hdt_file.search_triples('', broaderTransitive, n)
    if s_cardinality == 0:
        return True

def all_subclass_removed(n):
    (triples, cardinality) = hdt_file.search_triples('', broaderTransitive, n)
    flag  = True
...
def init_nodes():
    ...
    global can_remove
    (subclass_triples, cardinality) = hdt_file.search_triples('', broaderTransitive, '')
...
def construct_graph():
    ...
    for n in collect_nodes:
        (subclass_triples, cardinality) = hdt_file.search_triples('', broaderTransitive, n)
        for (s, _, _) in subclass_triples:
            if s in collect_nodes:
                if s != n :
                    graph.add_edge(s, n)
        (subclass_triples, cardinality) = hdt_file.search_triples(n, broaderTransitive, '')
...
def compute_strongly_connected_component():
    ...
    file =  open('broaderTransitive_node_color.csv', 'w', newline='')
    ...
    # export the edges
    file =  open('broaderTransitive_edges.csv', 'w', newline='')
    writer = csv.writer(file)
    ...
    # export the edges
    file =  open('broaderTransitive_edges_shortname.csv', 'w', newline='')
    writer = csv.writer(file)
    writer.writerow([ "Subject", "Object"])
    for (l,r) in graph.edges:
...
def draw_graph():
    ...
    # nx.draw(graph)
    plt.savefig("broaderTransitive_all_graph.png")
    plt.show()
