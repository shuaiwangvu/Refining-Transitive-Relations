# SUBMASSIVE 2
# analysis of the subgraph by broader
...
PATH_LOD = './narrowerTransitive.hdt'
...
def is_leaf_node (n):
    (_, s_cardinality) = hdt_file.search_triples('', narrowerTransitive, n)
    if s_cardinality == 0:
        return True

def all_subclass_removed(n):
    (triples, cardinality) = hdt_file.search_triples('', narrowerTransitive, n)
    flag  = True
...
def init_nodes():
    ...
    global can_remove
    (subclass_triples, cardinality) = hdt_file.search_triples('', narrowerTransitive, '')
...
def construct_graph():
    ...
    for n in collect_nodes:
        (subclass_triples, cardinality) = hdt_file.search_triples('', narrowerTransitive, n)
        for (s, _, _) in subclass_triples:
            if s in collect_nodes:
                if s != n :
                    graph.add_edge(s, n)
        (subclass_triples, cardinality) = hdt_file.search_triples(n, narrowerTransitive, '')
...
def compute_strongly_connected_component():
    ...
    file =  open('narrowerTransitive_node_color.csv', 'w', newline='')
    ...
    # export the edges
    file =  open('narrowerTransitive_edges.csv', 'w', newline='')
    writer = csv.writer(file)
    ...
    # export the edges
    file =  open('narrowerTransitive_edges_shortname.csv', 'w', newline='')
    writer = csv.writer(file)
    writer.writerow([ "Subject", "Object"])
    for (l,r) in graph.edges:
...
def draw_graph():
    ...
    # nx.draw(graph)
    plt.savefig("narrowerTransitive_all_graph.png")
    plt.show()
